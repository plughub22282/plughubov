...
export function PlayerBar(): React.ReactElement | null {
  ...
  return (
    <div
      data-tour="player"
      className="no-drag fixed bottom-4 z-30 flex items-center gap-3 rounded-2xl border border-app-border/50 px-3 py-2.5 shadow-[0_12px_38px_rgb(0_0_0_/_0.35)]"
      style={{
        left: 'calc(196px + (100vw - 196px) / 2)',
    ...
    >
      {/* Трек */}
      <div className="flex w-40 min-w-0 flex-shrink-0 items-center gap-2">
        <ImageWithFallback
          src={current.iconUrl}
          alt={current.title}
          seed={current.id}
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-2xs text-txt-muted">{current.author}</p>
        </div>
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col items-center justify-center gap-1.5">
        <button
          ...
          className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-accent text-[rgb(var(--btn-primary-text))] shadow-[0_4px_12px_rgb(0_0_0_/_0.3)] transition hover:scale-105 hover:shadow-[0_4px_16px_rgb(0_0_0_/_0.4)]"
        >
          {playing ? <Pause size={13} fill="currentColor" /> : <Play size={13} fill="currentColor" className="ml-0.5" />}
        </button>
        <div className="flex w-full min-w-0 items-center gap-1.5">
          <span className="w-8 flex-shrink-0 text-right text-2xs tabular-nums text-txt-muted">{fmt(currentTime)}</span>
          <button
            type="button"
            onPointerDown={seek}
      ...
      {/* Громкость — компактный слайдер фиксированной ширины (как в эталоне);
          w-16 + min-w-0 не дают нативному range с его дефолтной шириной ~129px
      ...
      <div className="flex flex-shrink-0 items-center gap-1.5 pr-1">
        ...
        <input
          ...
          className={`h-1 w-16 min-w-0 flex-shrink-0 cursor-pointer appearance-none rounded-full bg-zinc-700 transition-all ${sliderThumbClasses}`}
